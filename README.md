...
uv run --with pytest --with pytest-asyncio python -m pytest tests/ -q 2>&1 | tail -3 && git add -A && git commit -m "Release v1.4.1" && git tag v1.4.1 && git push origin main --tags
```
...
> **Notă**: HACS folosește tag-urile Git ca versiuni. Tag-ul trebuie să coincidă exact cu `version` din `manifest.json` (ex. ambele `1.3.6`, nu `v1.3.6` vs `1.3.6`).
